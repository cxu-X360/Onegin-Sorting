...
int text_beater1(char** text_ptr, char* string_pointers[])
{
	size_t pos = 0;
...
int comp_rev_alphabet_str(void* a_ptr, void* b_ptr)
{
	if (a_ptr == NULL || b_ptr == NULL)
	{
		fprintf(stderr, "NULL PTR ERROR\n");
...
int main()
{
	const char input_filename[] = "Texts\\Eugeny Onegin.txt";
	FILE* poem_file = fopen(input_filename, "r");
	NULL_FILE_ERROR(poem_file);

	char* text = NULL;

	size_t num_of_strings = 1000; //MAX_NUM_STRIGS
	char** string_pointers = (char**) calloc(num_of_strings, sizeof(char*));
